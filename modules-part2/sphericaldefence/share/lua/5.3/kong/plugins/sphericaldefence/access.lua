local JSON = require "kong.plugins.sphericaldefence.json"
...
local function parse_url(host_url)
  ...
  if not parsed_url.port then
    --[[if parsed_url.scheme == HTTP then
     ...
     parsed_url.port = 9000
  end
  if not parsed_url.path then
    parsed_url.path = "/evaluate"
  end
...
function _M.execute(conf)
  ...
  local name = "[sphericaldefence] "
  local ok, err
  local parsed_url = parse_url(conf.url)

  ngx.log(ngx.NOTICE, "[sphericaldefence]: URL - " .. conf.url)

  local host = parsed_url.host
  local port = tonumber(parsed_url.port)
  local payload = _M.compose_payload(parsed_url)

  ngx.log(ngx.NOTICE, "[sphericaldefence]: Payload - " .. payload)
...
if conf.blocking then
  ...
  if not body then
      ngx.log(ngx.ERR, name .. "no body received.")
  end
...
if body then
  ...
  ngx.log(ngx.NOTICE, "[sphericaldefence]: " .. body)
  -- Reject
  if response_body["isAnomaly"] then
    
    return kong.response.exit(HTTP_FORBIDDEN, {
        message = "This request contains an anomaly"
      })
...
function _M.compose_payload(parsed_url)
    ...
    local uri = JSON:encode(ngx.var.request_uri)
    local method = JSON:encode(ngx.var.request_method)
    ...
    local payload_body = [[{"headers":]] .. raw_json_headers .. [[,"method":]] .. method .. [[,"uri":]] .. uri .. [[,"uri_args":]] .. raw_json_uri_args.. [[,"body_data":]] .. raw_json_body_data .. [[}]]
