...
-- @author logronoide
-- 
-- New flag to control STALE state behaviour
local STALE_DATA_UPDATE = 0 	    -- Standard use. Recover Stale data when external fails
local STALE_DATA_CLEAN = 1	    -- Delete local data when external fails
local DEFAULT_STALE_BEHAVIOUR = STALE_DATA_CLEAN
...
local function load(self, key)
   ...
   if data then
      ...
      ngx.log(ngx.DEBUG, 'external lookup failed: ', err)
   end

   -- external lookup failed
   -- attempt to load stale data
   data, flags = _get_stale(self)
   if (DEFAULT_STALE_BEHAVIOUR == STALE_DATA_UPDATE) and data and not _is_empty(data, flags) then
