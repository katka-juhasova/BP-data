...
local aws_v4 = require "kong.plugins.aws-lambda-status-code.v4"
...
local AWSLambdaStatusCodeHandler = BasePlugin:extend()

function AWSLambdaStatusCodeHandler:new()
  AWSLambdaStatusCodeHandler.super.new(self, "aws-lambda-status-code")
end

function AWSLambdaStatusCodeHandler:access(conf)
  AWSLambdaStatusCodeHandler.super.access(self)
  ...
  if not upstream_body_json then
    ngx.log(ngx.ERR, "[aws-lambda-status-code] could not JSON encode upstream body",
  ...
  then
    ...
    if content_type:find("application/json", nil, true) then
      ...
      local t_body   = params[conf.transform_json_property]
