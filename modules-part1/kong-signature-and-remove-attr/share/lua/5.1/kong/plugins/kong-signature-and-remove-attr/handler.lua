...
local Signature = BasePlugin:extend()
local kong = kong
local string = string
local pairs = pairs


Signature.VERSION = "0.1.0-1"
Signature.PRIORITY = 1010

function Signature:new()
    Signature.super.new(self, 'kong-signature-and-remove-attr')
end
...
function Signature:access(conf)

    Signature.super.access(self)




    local body = parseBody(conf)
    local filter = {}

    for _,v in pairs(body) do

        if isExist(conf.remove_attr, _) == false then
            filter[_] = v
        end
    end
    ...
    local signature = createSignature(conf.signature_secret_key, filter, conf)

    filter["signature"] = signature



    if string.lower( kong.request.get_method() ) == "get" then
        kong.service.request.set_query(filter)
    elseif string.lower( kong.request.get_method() ) == "post" then
        -- kong.service.set_header("application/x-www-form-urlencoded")
        kong.service.request.set_body(filter)
    end
