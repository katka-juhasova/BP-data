...
local function get_queue_id(conf)
  return fmt("%s:%s:%s:%s:%s:%s:%s:%s:%s",
             conf.http_endpoint,
             table.concat(conf.body_content_types, ","),
             conf.method,
             conf.content_type,
             conf.timeout,
             conf.keepalive,
...
local function is_content_type(type, check)
  return type:sub(1, #check) == check
end

local function is_form_body(type)
  type = lower(type)
  return is_content_type(type, "application/x-www-form-urlencoded")
end

local function is_text_body(type)
  type = lower(type)
  return is_content_type(type, "text/") or is_form_body(type) or is_json_body(type)
end

local function is_binary_body(type)
  return not is_text_body(type)
end

local function should_capture_body(conf, type)
  if type then
    type = lower(type)
    for i, value in ipairs(conf.body_content_types) do
      if is_content_type(type, value) then
        return true
      end
    end
  end
...
local function parse_body(type, data)
  if type and data then
    if is_json_body(type) then
      return { type = "json",   data = cjson_decode(data) }
    elseif is_binary_body(type) then
      return { type = "binary", data = b64(data) }
    else
...
function HttpLogHandler:access(conf)
  if should_capture_body(conf, kong.request.get_header("Content-Type")) then
...
function HttpLogHandler:body_filter(conf)
  if should_capture_body(conf, kong.response.get_header("Content-Type")) then
    local ctx = kong.ctx.plugin;
