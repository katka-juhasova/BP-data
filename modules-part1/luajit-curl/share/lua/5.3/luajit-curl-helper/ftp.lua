...
function setCurlOpt(self, mode, opt)
	...
	if mode == "upload" then
		local url = "ftp://" .. password_auth .. self._opt.remote_addr .. ":" .. self._opt.port .. "/" .. opt.remote_dir
		...
		if fp ~= nil then
			...
			error("File Not Found. => " .. opt.target_file_dir)
		end
	...
	elseif mode == "remove" then
		...
		local command = "dele " .. opt.remote_dir
	...
	elseif mode == "rename" then
		...
		local command = "RNFR " .. opt.remote_dir

		self:log("command", command)

		table.insert(quote_list, command)

		command = "RNTO " .. opt.name
...
function _M.init(opt)
	...
	self._opt.port = self._opt.port or 21
